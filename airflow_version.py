...
def test_if_return_empty(return_dict:dict) ->bool:
    if len(return_dict) == 0:
        return False
    else:
        return True

def test_if_empty(main_df) -> bool:
    df = main_df.copy()
    return df.empty
@dag(
    dag_id='spotify_data_collector',
    start_date=dt.datetime(2022, 26, 11),
    max_active_runs=1
     )
def spotify_data_collector():
    @task
    ...
    @task
    def download_album_data(recently_played:dict):
        album_data = []
        ...
        return album_df
    @task
    def download_artist_data(recently_played:dict):
        artist_data = []
        for row in recently_played['items']:
        ...
        return artist_df
    @task
    def download_song_data(recently_played:dict):
        song_data = []
        for row in recently_played['items']:
    ...
    @task
    def df_cleanup(album_df, song_df, artist_df):
        # Clear up data to be only from today.
        list_of_items = []
        song_df = song_df[song_df['track_id'].isin(
            song_df[pd.to_datetime(song_df['played_at']).dt.date == dt.now().date()]['track_id'])]
        if song_df.empty:
        ...
        list_of_items.append(album_df)
        list_of_items.append(song_df)
        list_of_items.append(artist_df)
        return list_of_items

    @task
    def create_tables(metadata, list_of_items):
        ...
        with engine.connect() as conn:
            ...
            list_of_items[0].to_sql("albums_TEMP", con=engine, if_exists='append', index=False,chunksize=500)
            list_of_items[2].to_sql("artists_TEMP", con=engine, if_exists='append', index=False,chunksize=500)
            list_of_items[1].to_sql("songs_TEMP", con=engine, if_exists='append', index=False,chunksize=500)
